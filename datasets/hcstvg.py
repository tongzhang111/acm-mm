...
class VideoModulatedSTGrounding(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if images_list.shape[0] < 100:
            a = np.zeros([100,images_list.shape[1],images_list.shape[2],images_list.shape[3]])
            a[0:images_list.shape[0],:,:,:] = images_list
            images_list = a
        if images_list.shape[0] > 100:
...
def build(image_set, args):
    ...
    if args.test or image_set == "val":
        if not args.v2:  # only a test set
            ann_file = Path(args.hcstvg_ann_path) / f"test_procv1.json"
        else:  # only a val set
            ann_file = Path(args.hcstvg_ann_path) / f"val_proc.json"
    else:
        if not args.v2:
            ann_file = Path(args.hcstvg_ann_path) / f"train_procv1.json"
        else:
