...
def train_one_epoch(
    ...
    for i, batch_dict in enumerate(
        ...
        mmm = samples.decompose()
        meat_info[0,0] = mmm[1].shape[1]
        meat_info[0, 1] = mmm[1].shape[2]
        meta_info = {
        ...
        keep = torch.tensor(keep_list).long().cuda()
        ...
        outputs["hs_feature"] = outputs["hs_feature"][:, keep, :]
        ...
        # outputs["vita_pred_logits"] = outputs["vita_pred_logits"][:,keep,:]
        for i_aux in range(len(outputs["aux_outputs"])):
            outputs["aux_outputs"][i_aux]["pred_logits"] = outputs["aux_outputs"][i_aux][
                "pred_logits"
            ][:,keep,:]
        b = len(durations)
        targets = [
        ...
        assert len(targets) == len(outputs["pred_boxes"][0]), (
        ...
        loss_dict = {}
        if criterion is not None:
            loss_dict.update(criterion(outputs, targets, inter_idx, time_mask, keep))
...
def evaluate(
    ...
    for i_batch, batch_dict in enumerate(
        ...
        pred_scores = outputs["pred_logits"].transpose(0,1).sigmoid()  # [t, q, k]
        pred_scores = pred_scores.mean(0)  # [q, k]
        max_scores, _ = pred_scores.max(-1)  # [q,]
        _, max_ind = max_scores.max(-1)  # [1,]
        # max_inds = max_ind.repeat(outputs["pred_logits"].shape[0])
        if args.test:
        #     pred_boxes_all = outputs["pred_boxes"][:, max_ind, :].unsqueeze(1)
        #     targets_all = [x for x in targets]
        # outputs["pred_boxes"] = outputs["pred_boxes"][:, max_ind, :]
        # outputs["pred_sted"] = outputs["pred_sted"][:, max_ind, :].unsqueeze(0)
            pred_boxes_all = outputs["pred_boxes"][max_ind].unsqueeze(1)
            targets_all = [x for x in targets]
        outputs["pred_boxes"] = outputs["pred_boxes"][max_ind]
        outputs["pred_sted"] = outputs["pred_sted"][max_ind].unsqueeze(0)
        for i_aux in range(len(outputs["aux_outputs"])):
            pred_scores = outputs["aux_outputs"][i_aux]["pred_logits"].transpose(0,1)  # [t, q, k]
            pred_scores = pred_scores.mean(0)  # [q, k]
            max_scores, _ = pred_scores.max(-1)  # [q,]
            _, max_ind = max_scores.max(-1)  # [1,]
            # outputs["aux_outputs"][i_aux]["pred_boxes"] = outputs["aux_outputs"][i_aux][
            #                                                   "pred_boxes"
            #                                               ][:, max_ind, :]
            # outputs["aux_outputs"][i_aux]["pred_sted"] = outputs["aux_outputs"][i_aux][
            #                                                   "pred_sted"
            #                                               ][:, max_ind, :].unsqueeze(0)
            outputs["aux_outputs"][i_aux]["pred_boxes"] = outputs["aux_outputs"][i_aux][
                                                              "pred_boxes"
        ...
        if "vidstg" in postprocessors.keys():
            ...
            if args.sted:
                pred_steds = postprocessors["vidstg"](
                    outputs, frames_id, video_ids=video_ids, time_mask=time_mask, durations=durations
                )
            ...
            if args.sted:
                pred_steds = postprocessors["hcstvg"](
                    outputs, frames_id, durations, video_ids=video_ids, time_mask=time_mask
                )
            image_ids = [t["image_id"] for t in targets]
            for im_id, result in zip(image_ids, results):
