...
# load config
with open("/data/zhangtong1/tubedetr/config/vidstg.json", "r") as f:
    cfg = json.load(f)
video_path = os.path.join(cfg.get("vidstg_vid_path"), "video")
ann_path = cfg.get("vidstg_ann_path")
vidor_path = cfg.get("vidstg_vid_path")
...
for dir in vidor_dirs:
    ...
    for subdir in tqdm(subdirs):
        ...
        for file in files:
            if str(7003314632) in file:
                continue
            if str(9731127827) in file:
                continue
...
for file in files:
    ...
    for annot in tqdm(annotations):
        if str(7003314632) in annot["vid"]:
            continue
        if str(9731127827) in annot["vid"]:
            continue
