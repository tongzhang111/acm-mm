    ...
    def forward(
        ...
        if encode_and_save:
            ...
            if self.use_time_embed:  # add temporal encoding to init time queries
                # time_embed = self.time_embed(t).repeat(n_queries, b*t, 1)
            ...
            if isinstance(text[0], str):
                ...
                # words = self.tokenizer.tokenize(text[0])
                # words = [self.SPECIAL_TOKEN["CLS_TOKEN"]] + words
                # words = words + [self.SPECIAL_TOKEN["SEP_TOKEN"]]
                # input_ids = torch.tensor(self.tokenizer.convert_tokens_to_ids(words)).unsqueeze(0).cuda()
                # # encoded_text_1 = self.text_encoder_1(**tokenized)
                # text_memory = self.text_encoder(input_ids).transpose(0,1)
                # text_attention_mask = torch.zeros((1,input_ids.shape[1]), dtype=torch.float).bool().cuda()
            ...
            if self.return_weights:
                hs, weights, cross_weights, query_feature, content_query = hs
            ...
            if not self.return_weights:
                return hs, query_feature, content_query
            else:
                return hs, weights, cross_weights, query_feature, content_query
...
class TransformerDecoder(nn.Module):
    ...
    def forward(
        ...
        intermediate_query_feature = []
        intermediate_content_query_feature = []

        for i_layer, layer in enumerate(self.layers):
            output, weights, cross_weights, query_feature, content_query = layer(
                output,
                memory,
                tgt_mask=tgt_mask,
                memory_mask=memory_mask,
            ...
            if self.return_intermediate:
                ...
                intermediate_query_feature.append(query_feature)
                intermediate_content_query_feature.append(content_query)
                if self.return_weights:
        ...
        if self.norm is not None:
            ...
            if self.return_intermediate:
                intermediate.pop()
                intermediate_query_feature.pop()
                intermediate_content_query_feature.pop()
        ...
        if self.return_intermediate:
            if not self.return_weights:
                return torch.stack(intermediate), torch.stack(intermediate_query_feature), torch.stack(intermediate_content_query_feature)
            else:
                return (
                    torch.stack(intermediate),
                    torch.stack(intermediate_weights),
                    torch.stack(intermediate_cross_weights),
                    torch.stack(intermediate_query_feature),
                    torch.stack(intermediate_content_query_feature),
                )

        if not self.return_weights:
            return output, query_feature, content_query
        else:
            return output, weights, cross_weights, query_feature, content_query
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        content_query = self.norm2(tgt2.transpose(0,1))
        ...
        tgt = self.norm4(tgt)
        tgt = tgt.transpose(0,1)
        return tgt, weights, cross_weights, memory, content_query
